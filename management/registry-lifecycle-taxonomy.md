...
6. Once both `category:` and `lifecycle:` are annotated across all 342
   ...
   **Implemented 2026-06-12.** `render_pqc_lifecycle_counts()` in
   `scripts/generate_status_tables_from_yaml.py` emits an autogenerated
   "Post-Quantum Algorithm Counts by Lifecycle" table (markers
   `pqc-lifecycle-counts`) into `cryptographic-algorithms.md`, bucketing
   `cr-pqc.yaml` entries by `function × lifecycle` per the
   [Query Examples](#query-examples-for-summary-counts-pqc-rows) above and
   counting parameter-set instantiations (`oidMap` length → `parameterSet`
   values → 1). Result: KEMs standardised/selected 6, KEMs candidate/broken
   29, signatures standardised 30, draft 2, candidate 26, broken 4 (97
   total). The five hand-maintained PQC rows were consolidated into two
   catalogue-count rows in the Summary Counts table (preserving its `~426`
   total); the lifecycle detail now lives only in the autogenerated table.
   Note the **counting-unit divergence**: the autogen counts YAML
   parameter-set instantiations (97) whereas the catalogue collapses some
   (Classic McEliece `f`-variants, FAEST `s`/`f` × levels) into representative
   rows (93) — this is documented in-table rather than reconciled, since the
   two views answer different questions. The Round 2 / Round 3 editorial split
   present in the old rows is dropped (round granularity remains a
   [Deferred Decision](#deferred-decisions)); adding `candidate/pqc-round-N`
   path values would let it be reinstated as a further `lifecycle` sub-axis.
