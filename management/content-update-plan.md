...
> [!NOTE]
> **Currency verification (2026-07-11).** Web-checked all ingested sources for updates/addenda. **Confirmed current (no action):** SP 800-227 (KEM) — we hold the final (2025-09-18); composite-sigs still `-19` (pre-RFC); FIPS 206 (FN-DSA) still IPD; SP 800-57 Pt 1 Rev 6 still IPD; SP 800-131A Rev 3 still IPD (Rev 2 remains the current final); BSI TR-02102 v2026-01 current. **Two movers found:** (1) **DMTF SPDM → v1.4.0** (we had ingested 1.3.0) — adds PQC to NEGOTIATE_ALGORITHMS (ML-DSA/ML-KEM/SLH-DSA per FIPS 203/204/205; no hybrid until 1.5, feedback due 2026-08-31); `DSP0274_1.4.0.pdf` downloaded to `.downloads/`, being ingested as a delta over `cr-spdm.yaml`. (2) **TCG Algorithm Registry "Version 2.0" (2025-07-28)** newer than our Rev 1.35 (2025-02-18) — **ingested 2026-07-11** (PDF now in `.downloads/`): v2.0 is a rebrand+restructure — migrated the mnemonic prefix `TPM_` → `TCG_` (values unchanged). The `TCG_` name is now the primary `identifier`, the legacy `TPM_` name (still used by the TPM Library Spec / SPDM) kept in `legacyMnemonicId` on all 56 overlays; the `tcg:` overlay `registry` field was renamed to `section` with de-prefixed values (`algorithm`, `ecc-curve`, `*-parameter-set`). Added PQC registries (`TCG_ALG_MLKEM/MLDSA/HASH_MLDSA/SLHDSA/HASH_SLHDSA` + parameter-set tables → `tcg:` overlays on ML-KEM/ML-DSA/HashML-DSA/SLH-DSA/HashSLH-DSA), and marked ECB TCG Legacy (`authorities.tcg: deprecated`). Check 17 extended for the new sections, `legacyMnemonicId`, and shared parameter-set mnemonicIds. **Worth verifying:** FIPS 207 (HQC) IPD may now be released (would move HQC `lifecycle: selected → draft`); NIST IR 8547 — we hold the IPD, a final may exist (low impact, governance/roadmap).

> [!NOTE]
> **Currency verification (2026-07-12).** Targeted sweep of the two SBOM crypto registries and the NIST PQC programme. **All three current — no registry ingestion required.**
> - **CycloneDX** — spec advanced to **1.7.1 (2026-06-02)**; CBOM was expanded in **1.7 (2025-10-21)**, but that is BOM-schema structure, not the algorithm vocabulary. `cryptography-defs.json` (living catalogue on `master`, 98 families / 162 variant patterns) last changed **2026-05-21**; the substantive algorithm edits since our last sync — **SSH-KDF** (2026-03-27), **BLS12 pattern fix**, **GOST28147 name correction**, and **IKE-PRF `hashAlgorithm`** (#873) — are **already covered** in `cr-cdx.yaml` + `CycloneDxRegistryCoverageTest` (the test already uses the `[-{hashAlgorithm}]` forms). No new identifiers. *Pre-existing (non-currency) coverage gaps, deferred to Phase 2.1:* CycloneDX `TLS-PRF` family has no registry entry; the CycloneDX `SNOW3G` spelling is not in the coverage test (canonical `SNOW-3G` is present).
