...
- [x] **Category taxonomy field** ([`registry-category-taxonomy.md`](registry-category-taxonomy.md)) — added optional path-valued `category:` field to algorithm entries (336/342 = 98% annotated; 6 SPDX umbrella/non-crypto aliases intentionally skipped). Function-first hierarchy aligned with `cryptographic-algorithms.md` section structure (§1–§19); 26 leaf categories across 11 top-level branches (`symmetric/`, `hash/`, `mac`, `asymmetric/`, `hpke`, `kdf/`, `framework`, `rng/`, `composite`). `RegistryEntry.java` carries the field; `cr-*.yaml` schema docs updated; `scripts/generate_status_tables_from_yaml.py` emits an autogenerated "Algorithm Category Distribution" table inside `ae-pattern-validator/src/main/resources/registry/README.md`.
...
- [ ] Lifecycle field (companion to `category:`) — independent axis for standardisation lineage (`standardised | selected | candidate | broken | legacy | …`). Enables PQC-specific Summary Counts rows ("PQC KEMs — Round 3 candidates", etc.) as compound `category` × `lifecycle` queries. Vocabulary needs design (interaction with per-authority `nist.status` / `bsi.status` blocks must be analysed to avoid redundancy).
- [ ] Multi-category support for SPDX umbrella aliases (`spdx:rsa`, `spdx:gost`) via optional `categories: [list]` extension; currently 6 entries skipped (2 umbrellas + 4 unmappable like `spdx:dcc`, `spdx:ubi`, `spdx:uffizi`, `spdx:uxn`).
