...
genuinely span multiple branches and resist single-category placement, see
[The `"list"` Sentinel](#the-list-sentinel) below.
...
For entries that span multiple functional categories (typically SBOM umbrella
aliases that map to several canonical algorithms in different categories) or
have no clear single mapping, use the sentinel value `category: "list"`. This
keeps `category:` mandatory and unambiguous as a *single string field* without
introducing a multi-value `categories:` list. The validator treats `"list"`
as a valid vocabulary entry; the autogenerated category-distribution table
buckets these entries under their own `list` row.

Examples in current registry:

| Entry | Reason for `"list"` |
|:---|:---|
| `spdx:rsa` | Maps to RSAES-OAEP (encryption) *and* RSASSA-PSS / RSASSA-PKCS1 (signature) |
| `spdx:gost` | Maps to GOST-28147 (block cipher), GOSTR3410 (signature), GOSTR3411 (hash) |
| `spdx:ubi` | Skein/Threefish UBI mode — both block-cipher mode of operation and hash-construction primitive |
| `spdx:dcc`, `spdx:uffizi`, `spdx:uxn` | SPDX aliases without a clear standard cryptographic mapping |
