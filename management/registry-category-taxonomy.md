...
|:---|:---|:---|:---|
| `category` | string (`/`-separated path) | **mandatory** on every algorithm entry, single value | controlled (see [Vocabulary](#vocabulary)) |
...
`symmetric/block-cipher/mode**` or `**/aead` will find). For entries that
genuinely span multiple branches or have no clear standard cryptographic
mapping, see [Sentinel Values](#sentinel-values) below.
...
Two sentinel values keep the `category:` field mandatory and unambiguous as a
*single string field* without introducing a multi-value `categories:` list.
Each addresses a different reason a concrete category cannot be assigned:

#### `category: "unspecific"`

The identifier is *deliberately polysemous* — a single SBOM-naming-authority
identifier that maps to multiple canonical algorithms in *different functional
categories*. The entry is "many things" by design.
...
refuse to pick a single primary category because the entry's whole purpose is
being multi-valued.

#### `category: "unknown"`

The identifier has no clear standard cryptographic mapping. SBOM authorities
retain the name for backward compatibility but provide no specification we can
verify against.

| Entry | Reason |
|:---|:---|
| `spdx:dcc` | "SPDX Data Confidentiality Code" — no standard cryptographic definition found |
| `spdx:uffizi` | "SPDX Uffizi cipher" — not in the official SPDX cryptographic-algorithm-list |
| `spdx:uxn` | "SPDX UXN cipher" — not in the official SPDX cryptographic-algorithm-list |

We *don't* know what these are. There's no canonical algorithm to point to.

#### Why two sentinels and not one

The two values support distinct queries:

- **"Show me entries that need follow-up research."** → `category = "unknown"`. Just the genuine gaps; not the deliberately polysemous ones.
- **"Show me entries where my SBOM scanner should fan out to multiple canonical algorithms."** → `category = "unspecific"`. These have known fan-outs; `unknown` entries have nothing to fan out to.
- **"What is the registry's coverage of standard algorithms?"** → annotated entries with concrete categories + `unspecific` (intentional design choice) is *covered*; `unknown` is the addressable gap.
