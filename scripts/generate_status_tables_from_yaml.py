...
# ── PQC lifecycle counts (cryptographic-algorithms.md) ──────────────────────

PQC_YAML = REGISTRY_DIR / "cr-pqc.yaml"
ALGORITHMS_MD = REPO_ROOT / "cryptographic-algorithms.md"

# Each output row is (function-label, lifecycle-label, function-key,
# set-of-lifecycles). `function-key` is "KEM" (category == asymmetric/kem) or
# "SIG" (category startswith asymmetric/signature). Lifecycle groupings follow
# registry-lifecycle-taxonomy.md §Query Examples.
PQC_ROW_SPEC = [
...
def _pqc_instantiation_count(entry: dict) -> int:
...
def _pqc_function(entry: dict) -> str | None:
    cat = entry.get("category", "")
    if cat == "asymmetric/kem":
        return "KEM"
    if cat.startswith("asymmetric/signature"):
        return "SIG"
    return None


def render_pqc_lifecycle_counts() -> str:
    """Build the autogenerated PQC lifecycle-count table for
    cryptographic-algorithms.md from cr-pqc.yaml (the single source of truth).

    Counts parameter-set instantiations (see _pqc_instantiation_count) bucketed
    by function × lifecycle per PQC_ROW_SPEC. Composite entries (type composite,
    category composite) are excluded — they are counted in the separate
    "Composite / hybrid constructs" Summary Counts row.
    """
    entries = [e for e in load_entries(PQC_YAML) if e.get("type") == "algorithm"]

    # (function, lifecycle) -> summed instantiation count
    ...
    for e in entries:
        ...
        if fn is None:
            continue
        n = _pqc_instantiation_count(e)
        counts[(fn, e.get("lifecycle"))] += n
        grand += n
    ...
    accounted = 0
    for fn_label, lc_label, fn, lifecycles in PQC_ROW_SPEC:
        n = sum(v for (f, lc), v in counts.items() if f == fn and lc in lifecycles)
        accounted += n
        lines.append(f"| {fn_label} | {lc_label} | {n} |")

    # Any (function, lifecycle) instantiations not captured by the row spec would
    # indicate a new lifecycle value — surface them rather than silently drop.
    leftover = grand - accounted
    if leftover:
        lines.append(f"| PQC | other (update PQC_ROW_SPEC) | {leftover} |")

    lines.append(f"| **Total** | parameter-set instantiations | **{grand}** |")
    lines.append("")
    lines.append(
        "_Autogenerated from `ae-pattern-validator/.../registry/cr-pqc.yaml` by "
...
def main():
    ...
    sections_algorithms = {
        "pqc-lifecycle-counts": render_pqc_lifecycle_counts(),
    }
    ...
    for path, sections in [(PROTOCOL_STATUS_MD, sections_protocol),
                           ...
                           (ALGORITHMS_MD, sections_algorithms)]:
