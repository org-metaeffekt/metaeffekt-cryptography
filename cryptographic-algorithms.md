...
The two consolidated post-quantum rows above are expanded here by
standardisation lifecycle. This table is **autogenerated** from the `cr-pqc.yaml`
registry (the single source of truth) and replaces the previously
hand-maintained per-lifecycle PQC rows; regenerate it with
`python3 scripts/generate_status_tables_from_yaml.py`.

<!-- AUTOGEN:BEGIN pqc-lifecycle-counts -->
| Function | Lifecycle | Count |
|:---|:---|---:|
| KEMs | standardised / selected | 6 |
| KEMs | candidate / broken | 29 |
| Signatures | standardised | 30 |
| Signatures | draft | 2 |
| Signatures | candidate | 26 |
| Signatures | broken | 4 |
| **Total** | parameter-set instantiations | **97** |

_Autogenerated from `ae-pattern-validator/.../registry/cr-pqc.yaml` by `scripts/generate_status_tables_from_yaml.py`; counts parameter-set instantiations bucketed by `category × lifecycle` (see [`management/registry-lifecycle-taxonomy.md`](management/registry-lifecycle-taxonomy.md)). This is the YAML-faithful view and may exceed the consolidated post-quantum rows in the Summary Counts table above, which count representative catalogue rows: the registry enumerates instantiations (e.g. Classic McEliece systematic / `f` variants, FAEST `s` / `f` × levels) that the catalogue prose collapses._
<!-- AUTOGEN:END pqc-lifecycle-counts -->

---

## SPDX Coverage Notes

Cross-referenced against the [SPDX Cryptographic Algorithm List](https://github.com/spdx/cryptographic-algorithm-list/tree/main/yaml) (CC0 1.0, 127 entries; re-verified 2026-06-16). SPDX defines a flat enumeration of named algorithm identifiers used in SPDX SBOM documents. The 2026-06-16 resync found one upstream change since the April 2026 sync: SPDX renamed the mis-named `gost.yaml` to `gostr3412-2015.yaml` (2026-06-03) to match its `id` field — the identifier is the 64-bit Magma cipher (GOST R 34.12-2015, RFC 8891), not a generic GOST umbrella. The registry mapping was corrected from `spdx:gost` to `spdx:gostr3412-2015` → Magma accordingly.
